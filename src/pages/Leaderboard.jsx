...
const MOCK_USERS = [
    { id: 1, name: 'Alex Chen', role: 'Head', points: 1250, badge: 'Full Stack Wizard' },
    { id: 2, name: 'Sarah Jones', role: 'Coordinator', points: 1100, badge: 'UI/UX Master' },
    { id: 3, name: 'Mike Ross', role: 'Member', points: 950, badge: 'Backend Ninja' },
    { id: 4, name: 'Emily Blunt', role: 'Member', points: 880, badge: 'Data Scientist' },
    { id: 5, name: 'Jessica Day', role: 'Member', points: 850, badge: 'Frontend Dev' },
    { id: 6, name: 'Nick Miller', role: 'Coordinator', points: 1050, badge: 'DevOps Guru' }, // Intentionally out of order to test sort
    { id: 7, name: 'Winston Bishop', role: 'Member', points: 720, badge: 'Security Pro' },
    { id: 8, name: 'Cece Parekh', role: 'Member', points: 600, badge: 'Mobile Dev' },
];
                ...
                {restUsers.map((user, index) => (
                    <div key={user.id} className="rank-row animate-fade-in" style={{ animationDelay: `${0.5 + (index * 0.1)}s` }}>
                        ...
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            <div style={{ width: '40px', height: '40px', background: '#333', borderRadius: '50%' }}></div> {/* Avatar Placeholder */}
                            <div>
                                <div style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>{user.name}</div>
                                <div style={{ fontSize: '0.8rem', color: 'var(--neon-violet)' }}>{user.role}</div>
                            </div>
                        </div>
...
const TopCard = ({ user, rank, delay }) => {
    ...
    return (
        <div className="animate-fade-in" style={{
            ...
            <div style={{ width: '80px', height: '80px', background: '#333', borderRadius: '50%', margin: '0 auto 1rem', border: `2px solid ${isFirst ? 'var(--neon-cyan)' : '#555'}` }}></div>

            <h3 style={{ fontSize: '1.4rem' }}>{user.name}</h3>
            <p style={{ color: 'var(--neon-violet)', marginBottom: '1rem', fontSize: '0.9rem' }}>{user.role}</p>
